...
#Follow the XKCD model of selecting four random words and concatenating them together to for your password.
...
Password = (one + two.capitalize() + three + four.capitalize()) #The now defined Password, capitalizes the second and fourth words
better = Password.replace('o', '0', 3).replace('e', '3', 2).replace('t', '7', 1) #A new better password is generated and replaces the first three o for zeros.
...
my_password = "abcd" #Pre determined password for computer to guess
password_len = len(my_password) #Generates the length of the password for the program to guess in
guess_num = 0 #start the number of guesses at 0
done = False #Done starts as a False value

start= time.time() #sets the start time to the current time as a float in seconds.
stoptime = 10 #number of seconds after which the program should stop.
...
while not done: #As long as False the program will continue going
    ...
    guessed_pw = "".join([random.choice(string.ascii_lowercase)for n in range(password_len)]) #defines guessed_pw as a blank joined with random lowercase ascii with a total length equal to that of the password. Therefore a four letter password gerates guesses that are four in length.
